...
class unet(object):
    ...
    def __init__(self, channels_first=settings.CHANNELS_FIRST,
        ...
        self.custom_objects = {
            ...
            "fn_dice_ce_loss": self.fn_dice_ce_loss,
            "fn_dice_loss": self.fn_dice_loss,
            "fnfp_dice_loss": self.fnfp_dice_loss,
    ...
    def fn_dice_loss(self, target, prediction, axis=(1,2), smooth=0.0001):
        intersection = tf.reduce_sum(prediction * target, axis=axis)
        p = tf.reduce_sum(prediction, axis=axis)
        t = tf.reduce_sum(target, axis=axis)
        numerator = tf.reduce_mean(intersection + smooth)
        ...
        false_negatives = tf.cast(target > prediction, dtype=tf.float32)
        fn_penalty = tf.reduce_mean(false_negatives * self.penalty_weight)
        return dice_loss + fn_penalty
    
    def fn_dice_ce_loss(self, target, prediction, axis=(1,2), smooth=0.0001):
        false_negatives = tf.cast(target > prediction, dtype=tf.float32)
        fn_penalty = tf.reduce_mean(false_negatives * self.penalty_weight)
        
        return self.weight_dice_loss*self.dice_coef_loss(target, prediction, axis, smooth) + \
            (1-self.weight_dice_loss)*K.losses.binary_crossentropy(target, prediction) + \
            (1-self.weight_dice_loss)*fn_penalty

    def fnfp_dice_loss(self, target, prediction, axis=(1,2), smooth=0.0001):
        intersection = tf.reduce_sum(prediction * target, axis=axis)
        p = tf.reduce_sum(prediction, axis=axis)
        t = tf.reduce_sum(target, axis=axis)
        numerator = tf.reduce_mean(intersection + smooth)
        ...
        false_negatives = tf.cast(target > prediction, dtype=tf.float32)
        false_positives = tf.cast(prediction > target, dtype=tf.float32)

        fn_penalty = tf.reduce_mean(false_negatives) * 10
        fp_penalty = tf.reduce_mean(false_positives)

        return dice_loss + fn_penalty + fp_penalty
    
    def focal_dice_loss(self, target, prediction, axis=(1,2), smooth=0.0001, alpha=0.25, gamma=2.5):
        intersection = tf.reduce_sum(target * prediction, axis=axis)
        union = tf.reduce_sum(target + prediction, axis=axis)
        numerator = tf.constant(2.) * intersection + smooth
        denominator = union + smooth
        ...
        mean = tf.reduce_mean(coef)
        return -alpha * (1-mean)**gamma * tf.math.log(mean + smooth)
